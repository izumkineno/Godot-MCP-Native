...
Godot-MCP 实现了 **156 个工具**，分为 6 大类和 1 个 Guide 引导入口，另有两种启用级别：

**核心工具（30 个）**：默认启用，始终可见。
**补充工具（126 个）**：需在工具管理面板中手动启用，`mcp_start_here` 引导入口除外。

| 类别 | 核心 | 补充 | 总计 | 说明 |
|------|------|------|------|------|
| Guide Entry | 0 | 1 | 1 | 首读工具入口，用于快速选择开发、调试、运行时和健康检查路径 |
...
所有工具通过 `mcp_tool_classifier.gd` 进行分类管理，支持按分组批量启禁。工具默认根据分类设置启用状态：核心工具始终启用，补充工具初始禁用，少量通过注册参数显式标记的引导入口也会默认启用。
