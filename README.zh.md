...
- **全面的工具集**（163 个工具——30 核心 + 133 补充）：
  ...
  - **指南工具**（7 个补充项）：`mcp_start_here`、`list_groups` 和各分组指南（`node_guide`、`scene_guide`、`script_guide`、`editor_guide`、`debug_guide`、`project_guide`）帮助 agent 快速选出合适的工作流
...
### Guide (7 个补充项)
- `mcp_start_here` - 先阅读的引导指南，用于选择开发、调试、运行时辅助和项目健康检查路径
- `list_groups` - 列出所有工具分组及其引导工具
- `node_guide` - 节点创建、编辑、组、信号和场景审计指南
- `scene_guide` - 场景打开、保存、列出和关闭指南
- `script_guide` - 脚本读取、创建、修改、分析和搜索指南
- `editor_guide` - 编辑器状态、选择、导出和执行工具指南
- `debug_guide` - 调试、运行时检查和探针工作流指南
- `project_guide` - 项目设置、资源、测试、导入和健康检查指南
